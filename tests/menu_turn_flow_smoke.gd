...
func _test_building_system() -> void:
	...
	_expect(
		owner.money == money_before_build - APARTMENTS.build_cost,
		"Construction should deduct the selected building's configured cost."
	)
	_expect(
...
func _test_card_hand_and_additional_roll() -> void:
	...
	_expect(not fold_button.disabled, "Fold Early should be playable before rolling.")
	_expect(overtime_button.disabled, "Overtime should wait until after the first roll.")
	_expect(
		hospital_button.disabled,
		"Emergency Transfer should be disabled while the board has no hospital."
	)
	_expect(
	...
	_expect(
		...
		"Hand cards should use rounded rectangle presentation."
	)
	_expect(
	...
	_expect(
		not overtime_button.disabled,
		"Overtime should become playable after the first roll resolves."
	)
	overtime_button.pressed.emit()
	await process_frame
	_expect(
...
func _test_hospital_travel_card() -> void:
	...
	_expect(
		...
		"Hospital travel should not consume itself when no hospital exists."
	)
	_expect(
	...
	hospital.set_building(MEDIC_TOWER)
	...
	_expect(
		await game_manager.request_play_card(player, EMERGENCY_TRANSFER),
		"Emergency Transfer should resolve through the real board route."
	)
	_expect(
...
func _test_participant_count(participant_count: int) -> void:
	...
	if is_instance_valid(offered_plot):
		...
		_expect(
			...
			"A deed should show decision data without repeating its board index."
		)
