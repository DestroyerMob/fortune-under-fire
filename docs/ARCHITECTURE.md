...
| --- | --- | --- | --- |
| `GameScreen` | Scene assembly, participant setup, global Space/Escape routing | `get_selected_property()` compatibility delegate | Gameplay rules, UI implementation details |
...
| `CardHandController` | Bottom local hand, rounded card visuals, quantity and playability | card command submission, `refresh()` | Card consumption or effect resolution |
| `LocalPlayerHudController` | Local health/funds display and inline balance delta | `configure()` | Entity stats |
...
6. Resource IDs (`building_id`, `card_id`, and future plot IDs) are persistence
   identities. Display names are presentation and may change or be localized.
...
2. `GameManager` validates the active participant and publishes `dice_rolled`.
3. `Board.move_entity()` visits each route index and publishes movement events.
4. Passing route index zero awards lap income before the destination landing.
5. At the destination, `Plot.on_land()` can offer purchase or rent.
6. `BuildingEffectSystem.activate_landing()` resolves immediate attacks/healing.
7. `Board` publishes `plot_landed` and `movement_finished`; `GameManager`
   publishes `roll_finished`.
8. The participant can resolve the world action or press End Turn. End Turn
   automatically settles rent and declines an unanswered purchase.
9. `GameManager` publishes `turn_finished`, advances, and publishes the next
   `turn_started`.
...
1. `CardHandController` asks `GameManager.can_play_card()` for each visible
   local card and submits `request_play_card()` on click.
2. `GameManager` validates the active entity, phase, possession, pending landing
   action, and effect-specific requirements before consuming one card.
3. Fold Early publishes a typed result and finishes the unrolled turn.
4. Overtime grants one roll allowance after a completed roll; the shared action
   becomes Roll Again until that allowance is used.
5. Hospital Run asks `CardEffectSystem` for the nearest strictly-forward Medic
   ...
   movement without fabricating a dice roll.
6. `GameManager.card_played` publishes `CardPlayResult`; presentation refreshes
   but does not apply the outcome.

Cards cannot be played while movement resolves or a purchase/rent action is
pending. Hospital Run remains in the hand and disabled when no Medic Tower
exists. Its movement does not consume the normal roll.
...
- `PropertyGroupData`: shared group colour and legacy default value.
...
- `CardData`: stable `card_id`, type, effect type, copy, and presentation colour.
...
2. Add or select its stable effect enum and decide whether it modifies a command,
   performs route movement, reacts to a pass/landing, or is presentation-only.
...
5. The hand should read `can_play_card()` and submit the command only. Do not put
   effect rules in the button.
...
`tests/menu_turn_flow_smoke.gd`. It protects menu navigation, economy, all eight
...
and two-to-four participant turns.
...
- playable card effects separated from the rounded bottom-hand presentation,
  with typed `CardPlayResult` outcomes and route-authentic hospital movement.
...
4. Replace `building_activated` positional compatibility signals after all
   callers use `BuildingActivation`.
5. Extract match/turn state from the `GameManager` facade when networking or
...
6. Replace programmatic building meshes with authored building scenes while
   keeping `PlotPresenter` as the visual owner.
7. Pack the current UI node groups into reusable scenes after tests use
